        ...
        <div className="p-8 border-b border-slate-800/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              >
                <img
                  src="https://res.cloudinary.com/dtbqhmgjz/image/upload/v1764926597/employees/dev/documents/e8d94016-d909-48b7-add0-3e6a745eb67a-1764926594722-Logo%20Resolusi.png"
                  alt=""
                />
              </motion.div>
              <div>
        ...
        <nav className="flex-1 p-6 overflow-y-auto">
          ...
          >
            {visibleGroups.map((group, groupIndex) => (
              <div key={group.title}>
                <p className="px-2 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                  {group.title}
                </p>
                <motion.ul className="space-y-2" variants={menuVariants}>
                  {group.items.map((item, itemIndexInGroup) => {
                    ...
                    const flatIndex =
                      visibleGroups
                        .slice(0, groupIndex)
                        .reduce((n, g) => n + g.items.length, 0) + itemIndexInGroup;
                    return (
                      <motion.li key={item.id} variants={itemVariants} transition={{ delay: flatIndex * 0.04 }}>
                        <motion.button
                          onClick={() => {
                        ...
                        >
                          ...
                          />
                          <span className="font-medium relative z-10 text-left">{item.label}</span>
                        </motion.button>
                      </motion.li>
                    );
                  })}
